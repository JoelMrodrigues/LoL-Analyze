...
const DraftInterface = ({ draftData, onSave, onBack }) => {
  ...
  // Obtenir l'étape actuelle de la draft
  const getCurrentStep = () => {
    return currentDraft?.data?.currentStep || 0;
  };

  const getCurrentPhase = () => {
    const step = getCurrentStep();
    return DRAFT_SEQUENCE[step] || { type: 'completed', side: 'none', phase: 0 };
  };

  // Avancer dans la séquence de draft
  const nextStep = () => {
  ...
  // Sélectionner un champion (ban ou pick)
  const selectChampion = (championName) => {
    ...
    if (currentPhase.type === 'ban') {
      // Trouver le premier slot de ban libre
      ...
      // Trouver le premier slot de pick libre
...
// Composant temporaire pour le sélecteur (sera remplacé par le vrai dans l'étape suivante)
const ChampionSelector = ({ onSelectChampion, bannedChampions, pickedChampions, currentPhase, disabled }) => {
  const sampleChampions = ['Aatrox', 'Ahri', 'Akali', 'Alistar', 'Amumu', 'Anivia', 'Annie', 'Ashe'];
  ...
  return (
    <div className="p-6">
      <div className="mb-4">
        <h3 className="text-xl font-bold text-white mb-2">Sélection des Champions</h3>
        <p className="text-gray-400">
          {currentPhase.type === 'ban' ? 'Bannir' : 'Choisir'} un champion pour l'équipe{' '}
      ...
      <div className="grid grid-cols-8 gap-4">
        {sampleChampions.map(champion => {
          const isBanned = bannedChampions.includes(champion);
          const isPicked = pickedChampions.includes(champion);
          const isUnavailable = isBanned || isPicked;

          return (
            <button
              key={champion}
              onClick={() => !isUnavailable && onSelectChampion(champion)}
            ...
            >
              <div className="w-16 h-16 rounded border-2 border-gray-600 overflow-hidden">
                <img
                  src={`https://ddragon.leagueoflegends.com/cdn/14.1.1/img/champion/${champion}.png`}
                  alt={champion}
              ...
              <div className="text-xs text-center text-gray-300 mt-1">{champion}</div>
